...
// vdc: optional setting to unlock wallet for block minting only;
...
// vdc: total coins staked (non-spendable until maturity)
        ...
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it)
        {
            ...
            for (int i = 0; i < pcoin->vout.size(); i++)
            {
                if (pcoin->nTime > nSpendTime)
                    continue;  // vdc: timestamp must not exceed spend time
...

    {
        ...
        CTxDB txdb("r");
        {
            ...
            loop
            {
                ...
                // vdc: sub-cent change is moved to fee
                ...
                if (nChange > 0)
                {
                    ...
                    if (!GetBoolArg("-avatar")) // vdc: not avatar mode
                    {
...
// vdc: create coin stake transaction
...
// vdc: check 'spent' consistency between wallet and txindex
// vdc: fix wallet spent state according to txindex
...
// vdc: disable transaction (only for coinstake)
